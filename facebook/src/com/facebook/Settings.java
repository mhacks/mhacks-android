...
public final class Settings {
    ...
    static void publishInstallAsync(final Context context, final String applicationId,
        ...
        Settings.getExecutor().execute(new Runnable() {
            @Override
            public void run() {
                final Response response = Settings.publishInstallAndWaitForResponse(
                        applicationContext,
                        applicationId,
                        false);
                if (callback != null) {
    ...
    static Response publishInstallAndWaitForResponse(
        ...
        try {
            ...
            Utility.setAppEventAttributionParameters(publishParams,
                                                     identifiers,
                                                     Utility.getHashedDeviceAndAppID(context,
                                                                                     applicationId),
            ...
            if (lastPing != 0) {
                ...
                if (graphObject == null) {
                    return Response.createResponsesFromString("true",
                                                              null,
                                                              new RequestBatch(publishRequest),
                                                              true).get(0);
                }
                else {
    ...
    public static String getAttributionId(ContentResolver contentResolver) {
        try {
            ...
            Cursor c =
                    contentResolver.query(ATTRIBUTION_ID_CONTENT_URI, projection, null, null, null);
